...
export default function TenantChat() {
  ...
  const uploadImageToSupabase = async (uri: string): Promise<string | null> => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("chat-images").upload(name, buffer, { contentType: `image/${ext}` });
  ...
  const renderItem = ({ item }: any) => {
    ...
    return (
      <View style={{
        ...
        {item.image_url && (
          <Image source={{ uri: item.image_url }} style={{ width: 180, height: 180, borderRadius: 12, marginBottom: 6 }} resizeMode="cover" />
        )}
        {!!item.content && <Text style={{ color: isOwn ? "white" : "black", fontSize: 15 }}>{item.content}</Text>}
        <Text style={{
          fontSize: 10,
        ...
        <View style={{
          ...
          {landlord && (
            <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingTop: 10 }}>
              <Image source={{ uri: landlord.profile_pic_path || "https://i.pravatar.cc/150?u=default" }} style={{ width: 38, height: 38, borderRadius: 9999, marginRight: 10 }} resizeMode="cover" />
              <Text style={{ fontSize: 17, fontWeight: "bold" }}>{landlord.first_name} {landlord.last_name}</Text>
            </View>
          )}
        </View>
