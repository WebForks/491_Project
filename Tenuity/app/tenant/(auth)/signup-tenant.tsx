...
export default function SignupTenant() {
  ...
  const associateWithLandlord = async (tenantId) => {
    const { data: landlord, error: landlordError } = await supabase
      .from("Landlords")
      .select("user_id, tenants")
      .eq("email", landlordEmail.toLowerCase())
      .single();

    if (landlordError || !landlord) {
      Alert.alert("Invalid Email", "That landlord email does not exist.");
      return;
    }

    const landlordId = landlord.user_id;
    const updatedTenants = Array.isArray(landlord.tenants)
      ? [...landlord.tenants, tenantId]
      : [tenantId];

    const { error: updateLandlordError } = await supabase
      .from("Landlords")
    ...
    if (updateLandlordError || updateTenantError) {
      console.log(
        "Error updating associations:",
        updateLandlordError,
        updateTenantError,
      );
      Alert.alert("Error", "Failed to link tenant with landlord.");
    } else {
      console.log("Tenant successfully linked with landlord.");
    }
  };
  ...
  const handleRegister = async () => {
    ...
    if (error) {
      ...
      addUserToTenants(userEmail, userId);
    }
  };

  // Add authenticated users to the Tenants table
  const addUserToTenants = async (userEmail, userId) => {
    ...
    if (error) {
      console.log("Error while adding user to Tenants:", error);
    } else {
      console.log("User successfully has been added to Tenants table!", data);
      Alert.alert("Successfully created account!");
    }

    if (landlordEmail.trim()) {
      await associateWithLandlord(userId);
      // After account creation, navigate user to their dashboard
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 16 }}>
            ...
            <View className="border-2 border-[#38B6FF] rounded-lg p-4 mx-4">
              ...
              {/* Landlord Email */}
              <View className="mb-4">
                <Text className="text-left text-lg mb-1">Landlord Email</Text>
                <TextInput
                  value={landlordEmail}
